...
library(readxl)
catalogue <- read_xlsx("./catalogue/lwc2-catalogue.2020-07-29.protected.xlsx")
translated <- read_xlsx("./catalogue/lwc2-catalogue.2020-07-29.protected.zh.xlsx") %>% select(2)
```
...
for (.i in 1:nrow(catalogue)) {
  ...
  .target_text = translated[.i, 1]

  message(paste("TODO", .text_id, .a_text_id_last, .source_text, .target_text))

  if (.a_text_id_last == "welcome") {

    #lsSetLanguageProperties(surveyID = surveyID, lang = lang, properties = list(surveyls_welcometext = .target_text))

  } else if (.a_text_id_last == "policy_label") {

    #lsSetLanguageProperties(surveyID = surveyID, lang = lang, properties = list(surveyls_policy_notice_label = .target_text))

  } else if (.a_text_id_last == "policy_notice") {

    #lsSetLanguageProperties(surveyID = surveyID, lang = lang, properties = list(surveyls_policy_notice = .target_text))
